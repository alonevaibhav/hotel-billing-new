...
class MenuItemService {
  ...
  /// Filter items based on category, search, and filters
  /// Now supports searching by both item name and menu code
  static List<Map<String, dynamic>> filterItems({
    required List<Map<String, dynamic>> allItems,
    required String selectedCategory,
    required String searchQuery,
    ...
    var filtered = allItems.where((item) {
      ...
      // Search filter - now searches in both item_name and menu_code
      bool matchesSearch = true;
      if (searchQuery.isNotEmpty) {
        final query = searchQuery.toLowerCase().trim();
        final itemName = (item['item_name'] as String?)?.toLowerCase() ?? '';
        final menuCode = (item['menu_code'] as String?)?.toLowerCase() ?? '';

        // Match if either item name or menu code contains the search query
        matchesSearch = itemName.contains(query) || menuCode.contains(query);
      }
