...
class DoneOrderController extends GetxController {
  ...
  /// Accept order
  /// Accept order - Updates all merged items to 'ready' status
  Future<void> acceptOrder(BuildContext context, int orderId) async {
    try {
      isLoading.value = true;
      errorMessage.value = '';
      ...
      final orderIndex = ordersData.indexWhere((order) => order.orderId == orderId);
      if (orderIndex == -1) {
        developer.log('⚠️ Order #$orderId not found', name: 'DoneOrders');
        return;
      }

      final order = ordersData[orderIndex];

      // ✅ Collect ALL item IDs including merged ones
      final List<int> allItemIds = [];

      for (var item in order.items) {
        // Each item may contain multiple merged IDs
        allItemIds.addAll(item.mergedItemIds);

        developer.log(
          '📝 Item "${item.itemName}" contains ${item.mergedItemIds.length} merged IDs: ${item.mergedItemIds}',
          name: 'DoneOrders',
        );
      }

      developer.log(
        '🔄 Updating ${allItemIds.length} total items (including merged) to ready status',
        name: 'DoneOrders',
      );

      // Update status for ALL items (including merged ones)
      await _repository.updateAllOrderItemsStatus(
        orderId: order.orderId,
        itemIds: allItemIds, // ✅ Pass all merged item IDs
        status: 'ready',
      );

      developer.log(
        '✅ Successfully updated all ${allItemIds.length} items',
        name: 'DoneOrders',
      );
      ...
      // ✅ Remove the order immediately after successful update
      ordersData.removeAt(orderIndex);
      ordersData.refresh();

      developer.log(
        '✅ Order #$orderId removed from preparing list',
        name: 'DoneOrders',
      );
